...
// inside CompilationPolicy::initialize()
  ...
  const size_t alignment = MAX2(ps, (const size_t) os::vm_allocation_granularity());
...
CodeBlob* CodeCache::allocate(int size, int code_blob_type, bool handle_alloc_failure, int orig_code_blob_type) {
  ...
  if (code_blob_type == CodeBlobType::MethodHot && log_is_enabled(Debug, codecache)) {
...
float CodeCache::count_hot_method_sparsity() {
  ...
  log_debug(codecache)("hot methods occupy %i blocks out of %i ~ %f; hot methods in/out of the MethodHot segment: %i/%i, %i/%iKB",
